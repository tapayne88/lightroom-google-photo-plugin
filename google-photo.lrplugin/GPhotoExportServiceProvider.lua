...
local function updateCantExportBecause(propertyTable)
	if not propertyTable.validAccount then
		propertyTable.LR_cantExportBecause =
			LOC("$$$/GPhoto/ExportDialog/NoLogin=You haven't logged in to Google Photo yet.")
		return
	end
...
function exportServiceProvider.startDialog(propertyTable)
	...
	propertyTable:addObserver("validAccount", function()
	...
	return {
		{
			...
			f:row({
				...
				f:static_text({
					title = LOC(
						"$$$/GPhoto/ExportDialog/ForDevelopers=for Developers (If you want to use your own API Key)"
					),
					alignment = "left",
					fill_horizontal = 1,
				}),
...
function exportServiceProvider.processRenderedPhotos(functionContext, exportContext)
	...
	local progressScope = exportContext:configureProgress({
		title = nPhotos > 1 and LOC("$$$/GPhoto/Publish/Progress=Publishing ^1 photos to GPhoto", nPhotos)
	...
	logger:trace(
		string.format("processRenderedPhotos albumId:%s, isDefaultCollection: %s", albumId, isDefaultCollection)
	)
