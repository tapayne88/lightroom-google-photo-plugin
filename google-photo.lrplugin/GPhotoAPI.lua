...
local function formatError(_nativeErrorCode)
...
local function oauth_encode(value)
	return tostring(string.gsub(value, "[^-._~a-zA-Z0-9]", function(c)
...
local function generate_nonce()
	return LrMD5.digest(tostring(math.random()) .. tostring(LrDate.currentTime()) .. SALT)
end
...
local function call_it(consumer_key, consumer_secret, method, url, params, rid)
	...
	if method == "POST" then
		...
		return LrHttp.get(url .. "?" .. query_string, { auth_header, { field = "User-Agent", value = USER_AGENT } })
	end
end
...
local function auth_header(propertyTable)
	...
	return {
		...
		{
			field = "Authorization",
			value = "Bearer " .. propertyTable.access_token,
		},
	}
end
...
function GPhotoAPI.uploadPhoto(propertyTable, params)
	...
	if photoId then
		...
		LrErrors.throwUserError(
			LOC(
				"$$$/GPhoto/Error/API/Upload=GPhoto API returned an error message (function upload, message ^1)",
				"error message"
			)
		)
	end
...
function GPhotoAPI.refreshToken(propertyTable)
	...
	local response, headers =
		call_it(args.client_id, args.client_secret, "POST", ACCESS_TOKEN_URL, args, math.random(99999))
	logger:info("Refresh token response: ", response)
	if not response or not headers.status then
		LrErrors.throwUserError(
			"Could not connect to Google Photo. Please make sure you are connected to the internet and try again."
		)
	end
	local json = require("json")
...
function GPhotoAPI.login(context, consumer_key, consumer_secret)
	...
	if action == "cancel" or not properties.verifier then
		return nil
	end
	...
	if not response or not headers.status then
		LrErrors.throwUserError(
			"Could not connect to googleapis.com. Please make sure you are connected to the internet and try again."
		)
	end
